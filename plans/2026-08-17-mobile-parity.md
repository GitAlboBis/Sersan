...
- **A1** `case-studies-rail.tsx:304` (`stackInFlow` da `detected && coarse`, non da `mode`), **A2** `drag-rail.tsx:122/341/247` (tutto sotto `@media (pointer: coarse)`), **B2** `case-studies-client.tsx:99` (`tap-44 press-surface`), **B3** `services-section.tsx:895`.
- **§2** (pagina montata ≤768 poi allargata resta mobile): diagnosticare il latch (`hasDetectedViewport`/`gsap.matchMedia` che non reverte/altezza runway scritta una volta) prima di toccare.
...
- **Desktop stretto 700×900 fine pointer** e **desktop con `hardwareConcurrency=4` emulato** (DevTools → Sensors/override): `level 1`, stesso set di canvas/isole di `main`, tunnel 14k — byte-identical al comportamento attuale.
