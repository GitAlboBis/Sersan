...
| 4d, 6.4-6.5, Final | ⏳ | — | 4d stretch dietro `RAIL_ISLANDS_TOUCH` (≥1 g, sorgente continua da costruire) — dopo il gate |
...
Phase 2 `fxBudget.postFx="off"` · Phase 4b `HERO_BRAND_COMPACT=false` in `cinematic-system-scroll.tsx` · Phase 4c `SEQ.LITE_RAYMARCH=false` · Phase 4d `RAIL_ISLANDS_TOUCH=false` · Phase 4e gyro `GYRO_PARALLAX_ENABLED=false` in `src/lib/gyro-parallax.ts` (nota: `fxBudget.gyroParallax` nell'HUD è il *desiderio* del device, non lo stato vivo) · Phase 3.4 `SESSION_SHORT=false` · Phase 5 FLIP coarse `COARSE_MIN_VISIBLE` in `use-flip-source.ts` (>1 = mai) · **tutto insieme** (spegne 2/4a/4b/4c/4d/4e): `?fx=1` o `stepDownBudget()`.
