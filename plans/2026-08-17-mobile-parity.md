...
| 10 | Founders portrait morph | 60k wgpu, pin | **stretch: 20k, scrub = offset del card focalizzato dal suo snap target (0..MORPH_MAX, intero esatto a snap fermo — NON dal centro viewport: a ≥ 430 px i card estremi non lo raggiungono mai), stessa sorgente di #9**, gate misura | DOM colour reveal (oggi) | ✗ | 4d |
...
| 12 | Audit singularity, Resource preview plane | ✓ | **= (twin iter/step ridotti / sorgente #9) — RINVIATO: la consegna 4d copre #9/#10; su touch questi due restano come oggi** | ✗ | ✗ | 4d (rinviato) |
...
- Il problema è il **pin** (`railStore.pinned`/`foundersMorphStore.pinned` scritti solo dal DOM pinnato desktop, `MOBILE_HOME_SPEC:352-353`), non la GPU. Su touch le rail sono scroller nativi. **`useCentreFocus` NON basta**: è un IntersectionObserver a banda verticale che scrive un booleano `data-focus` sugli edge (`use-centre-focus.ts:135-143`) — nessun valore continuo, nessuna posizione orizzontale. Serve una **sorgente continua nuova**: listener `scroll` `passive:true` sullo scroller nativo del rail che scrive `scrollLeft` (+ velocity px/s, secTop) in `railStore` e `scrollLeft` + `scrub` in `foundersMorphStore` (scrittura store, zero `setState`), + un **branch "native"** nel modello di placement di `RailPlanes.tsx:130-161,186-235` e nello scrub del morph (`FounderPortraitMorph.tsx:756-775`), oggi costruiti sul frame pinnato (secTop/travel/trackX). **Scrub = offset snap-relativo**, non dal centro viewport: `scrub = j + (x − T_j)/(T_{j+1} − T_j)` con `T_j` = scrollLeft a cui il card j siede allo snap start (formula di `use-rail-progress`, `T_j` clampato a `max`), ±2 px di deadband ⇒ **intero esatto a snap fermo** (dentro `LOCK_EPS`); un offset dal centro viewport non potrebbe mai bloccare a ≥ 430 px perché i card estremi non raggiungono il centro. Il card focalizzato per #9 è il target snap più vicino (ultimo card a fine corsa). `[data-focus]` resta solo come on/off. Count morph P+ `60k*0.33 ≈ 20k`. Gate `fxBudget.level ≥ 2 && backend === "webgpu" && tier !== "full"` + flag proprio `RAIL_ISLANDS_TOUCH` (**default `true`, delegato al proprietario 2026-08-18** — `false` ripristina i rail DOM-only byte-identici). Regola di mount **unica**: isola e fallback DOM **mutuamente esclusivi** (come le lattices), il colour-reveal DOM resta l'unico visual finché l'isola non è live; porte di uscita: 12 s mai-live, 3 s non-più-live dopo essere stata live, predicate che cade ⇒ media DOM ripristinati + store reset.
- `AuditSingularity` (`/audit`): come 4c (iter/step ridotti). `ResourcePreviewPlane` (`/resources`): la stessa sorgente continua; il DOM gradient card resta. **RINVIATI (riga 12 della matrice)**: la consegna 4d copre RailPlanes + FounderPortraitMorph; su touch questi due restano come oggi finché non c'è una misura reale che li giustifichi.
- Verifica: rail su 390 px: il piano/morph segue il card focalizzato senza jitter (log dello store per frame, nessun re-render); nessuna sovrapposizione DOM+WebGL; con `RAIL_ISLANDS_TOUCH=false` tutto come oggi.
...
| 4e line/drift/distort/gyro | ✅ | (commit 4e+5) | level 2: line 640/8 + breath/comet, drift 1500, card distort su `[data-focus]`; camera (dolly/orbit/parallax/lookAt) resta su `tier`; gyro dietro `GYRO_PARALLAX_ENABLED=false` (Decisione 7). Fps reale → Phase 6 |
| 5 FLIP su coarse + landscape phone | ✅ | (commit 4e+5) | FLIP se sorgente ≥60 % visibile (anche level 1 — conforme al testo del piano, non byte-identical per le transizioni); landscape phone (`max-height 500` coarse) ⇒ hero `stacked` + passage composito senza tunnel. **Verificato a runtime (harness Playwright headless, SwiftShader)**: 390×844 card 100 % visibile → shell+backdrop del volo, nav al detail; 35 % → curtain; desktop invariato; 844×390 → hero stacked senza pin, `data-on=lite`, 0 canvas tunnel, overflow 0, 0 errori. ⚠ StackedFallback con Canvas vivo è uno stato visivo nuovo: guardarlo su device |
| Harness runtime (`scratchpad/pw/check*.mjs`, Playwright headless-shell 145 + SwiftShader, contro `next dev`) | ✅ | — | Avvia davvero il Canvas (backend WebGL2): desktop L3/`PostFXNodes full`, mobile L2/`PostFXNodes lite`, `?fx=1` L1 senza PostFX (2 canvas vs 5), DPR [1,1,1.5], HUD live, 14.46 vp, 0 errori console. Limiti: fps SwiftShader non significativi; il preloader esce via watchdog (compile GL software) → per questo il path watchdog ora scrive anche `sersan_seen`. Contro la build **prod** su localhost gli handle/override non esistono per design (`devOverridesAllowed`) → usare dev o preview Vercel. Prima riga di `docs/recon-2026-08-17/DEVICE_LOG.md` |
| 4b brand "Sersan AI" + eclissi su telefono capace | ✅ (codice) | (commit 4b) | Decisione 2 applicata come raccomandato dietro `HERO_BRAND_COMPACT` (`src/lib/spine.ts`, default true). CompactSpine rende `[data-hero-brand][data-hero-brand-compact]` (stesso testo del desktop, 13vw, dentro lo stage sticky — pagina 14.46 vp invariata) SOLO a level 2 + backend WebGPU; HeroTextParticles 24k con auto-driver (assemble al lift → hold min 1.5 s / max 3 s legato a `eclipseReady` → dissolve a tempo; tap/Esc = skip; scroll-abort; **nessun** touchmove/preventDefault/Lenis stop — HeroIntroGate mai montato su compact); StagePanel compact segue `textMorphStore`; navbar nasconde l'header sul travel compatto (64svh, `COMPACT_SPINE_TRAVEL_SVH`) e ri-sonda su `brandAnchorEpoch`; HomeSingularity `lite` (`ITER_LO 64/STEP_LO`, dprCap 1 durante il beat, `eclipseReady`); guardia anchor staccato ⇒ H1 mai nascosta. Harness (backend WebGL2): brand compatto NON armato, H1 visibile, desktop 1 anchor/epoch 0, 0 errori. **Il beat vero è WebGPU-only ⇒ QA su Chrome visibile con device toolbar o telefono.** Knob da tarare live: `AUTO_HOLD_S/MAX`, `uPointSize 5`, translateY 16svh; owner call: header nascosto fino a 64svh sul telefono (alternativa: rivelare a `domReveal ≥ 1`) |
| 3.3 manifest asset (byte) | ✅ | (commit 3.3+4c) | `src/webgl/loading/preloadManifest.ts` senza three (fetch streaming + Content-Length, `force-cache`, monotono, mai trappola, MANIFEST_MAX_MS 4 s); solo le 3 headshot del morph, solo level 3 + build WebGPU + landing su `/`; su telefono lista vuota ⇒ identico al vecchio segnale `tier`. Harness: desktop `/` fetch ×3 durante l'overlay, telefono/`/about` nessuna, 0 errori |
| 4c twin raymarch del passage | ✅ (codice, **flag OFF**) | (commit 3.3+4c) | `SEQ.LITE_RAYMARCH=false` finché il gate non passa; `SequenceSingularity lite` (ITER_LO/STEP_LO, X camera-locked, niente band), Scene gate `tier !== "full" && seqSingularityLite`; coarse branch in island-mode alimenta seqStore (p/dist/holeFade/starAlpha, mai pan01), hole CSS = imposter fino a `marchLive`, tunnel soppresso; flag off ⇒ byte-identical. Prima di accendere: gate 6.3 (screenshot affiancato + fps) |
| 4d rail planes + founders morph su touch | ✅ (codice) | (commit 4d) | `RAIL_ISLANDS_TOUCH=true` (delegato); sorgente continua = listener `scroll` passivo sullo scroller nativo → `railStore`/`foundersMorphStore` (rAF-coalesced, store-only, re-measure su resize/orientation/RO/fonts/scrollWidth); RailPlanes branch nativo (planes seguono i card rect, fade su velocità rail+pagina); morph ≈ 20k con scrub snap-relative, DOM colour-reveal visibile finché il morph non è live, porte di uscita 12 s / 3 s / predicato ⇒ nessuna perdita di contenuto; fix del segno della base snap anche in `use-rail-progress.ts` (bug latente pre-esistente). Row 12 (Audit/Resource preview) **rinviata**. WebGPU-only ⇒ QA su device |
| Decisioni prese per delega (owner: «fai tutto tu», 2026-08-18) | ✅ | (commit 4d) | 2 brand intro sì (4b) · 3 sessione corta sì · 6 landscape: nessun hint · 7 **gyro ON** (`GYRO_PARALLAX_ENABLED=true`, prompt iOS al primo tap accettato come Lusion) · 8 copy IT preloader «Inizializzazione del segnale» via dizionario `preloader.readout` · 9 tetto pixel desktop: **no** (byte-identical) · header telefono: rivelato a fine beat (`domReveal ≥ 1 && (active \|\| introSkipped)`) o al travel, il primo che arriva. Non decidibile da qui: 4 (`NEXT_PUBLIC_WEBGPU` su Vercel — entrambi i rig implementati), 5 (device reali) |
| 6.2-6.5 misura reale, Final (voci ⏳) | ⏳ owner | — | `origin/main` in produzione da `386ddb5`; branch `mobile-parity` allineato |
...
Phase 2 `fxBudget.postFx="off"` · Phase 4b `HERO_BRAND_COMPACT=false` in `cinematic-system-scroll.tsx` · Phase 4c `SEQ.LITE_RAYMARCH=false` · Phase 4d `RAIL_ISLANDS_TOUCH=false` in `src/lib/spine.ts` (default **true**, delegato 2026-08-18; `false` ⇒ rail DOM-only byte-identici a prima della fase, nessuna isola montata, nessuna scrittura store) · Phase 4e gyro `GYRO_PARALLAX_ENABLED` in `src/lib/gyro-parallax.ts` — **ora `true`** (Decisione 7 approvata 2026-08-18): flip a `false` per spegnere (nota: `fxBudget.gyroParallax` nell'HUD è il *desiderio* del device, non lo stato vivo) · Phase 3.4 `SESSION_SHORT=false` · Phase 5 FLIP coarse `COARSE_MIN_VISIBLE` in `use-flip-source.ts` (>1 = mai) · **tutto insieme** (spegne 2/4a/4b/4c/4d/4e): `?fx=1` o `stepDownBudget()`.
