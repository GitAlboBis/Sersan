...
/** Ignition ramp length (s) — inside-the-hole rework (owner 2026-08-28):
 *  the whole intro is the climb OUT of the black hole, so the light comes up
 *  slowly across BOTH exit gates — near-dark while the mark generates and
 *  the hole is merely glimpsed (gate 2), full exposure landing with the
 *  hero frame (gate 3). */
const IGNITE_S = 4.5;
/** Exposure floor while the chrome is up: NEAR BLACK on purpose — "la scena
 *  del preloader inizia a schermo nero: siamo dentro il buco nero". The
 *  emissive wordmark forming through bloom is the only light in there. */
const IGNITE_FLOOR = 0.1;
