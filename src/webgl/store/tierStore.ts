...
interface TierState {
   ...
   * resolves to the WebGL2 fallback backend. DOM features that require a true
   * compute backend to be driveable must gate on THIS, not on the flag —
   * otherwise they render a layout their island can never animate (the
   * founders morph left founder B permanently at opacity 0).
   *
