...
export interface ImageSampleSpec {
   ...
   * white shirt) is bright+neutral; dark hair/beard is dark+neutral — so this
   * removes the wall (no rectangular halo) while KEEPING the hair/beard, which
   * satFloor/lumFloor would wrongly cull. Undefined disables. Live-tunable.
   */
  bgLumCeil?: number;
...
export function sampleImagePoints(
  ...
  for (let y = 0; y < gridH; y += stride) {
    for (let x = 0; x < gridW; x += stride) {
      ...
      // Background isolation — two neutral-drop tests:
      ...
      //    — so this strips the backdrop (no rectangular halo) while KEEPING the
      //    hair and beard.
